...
io.on('connection', (socket) => {
  ...
  console.log('QR Code foi gerado?', qrCodeGenerated);

  // Enviar status atual para o cliente
  const isConnected = whatsappClient !== null && isAuthenticated;
  socket.emit('whatsapp-status', { 
    connected: isConnected, 
    status: isConnected ? 'ready' : (qrCodeGenerated ? 'qr_received' : 'disconnected')
  });
  ...
  socket.on('init-whatsapp', async () => {
    try {
      ...
      console.log('Status atual - Client:', !!whatsappClient, 'Auth:', isAuthenticated, 'Init:', isInitializing, 'QR:', qrCodeGenerated);
      ...
      whatsappClient.on('qr', async (qr) => {
        try {
          ...
          console.log('Estado atual - QR:', qrCodeGenerated, 'Auth:', isAuthenticated, 'Init:', isInitializing);
        } catch (error) {
      ...
      whatsappClient.on('ready', () => {
        ...
        isInitializing = false;
        qrCodeGenerated = false; // Limpar flag do QR
        socket.emit('whatsapp-status', { 
          connected: true, 
          status: 'ready' 
        });
        console.log('Estado final - QR:', qrCodeGenerated, 'Auth:', isAuthenticated, 'Init:', isInitializing);
      });
      ...
      whatsappClient.on('authenticated', () => {
        ...
        console.log('Estado após autenticação - QR:', qrCodeGenerated, 'Auth:', isAuthenticated, 'Init:', isInitializing);
      });

      console.log('Inicializando cliente...');
      await whatsappClient.initialize();
      console.log('Cliente inicializado com sucesso');
      console.log('Estado após inicialização - QR:', qrCodeGenerated, 'Auth:', isAuthenticated, 'Init:', isInitializing);
